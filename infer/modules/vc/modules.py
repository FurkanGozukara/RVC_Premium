...
class VC:
    ...
    def _model_not_loaded_error(self):
        if self.pipeline is None or self.net_g is None or self.tgt_sr is None:
            return (
                "Please select a voice model in 'Inferencing voice' and wait for it "
                "to finish loading before clicking Convert."
            )
        return None
    ...
    def get_vc(self, sid, *to_return_protect):
        ...
        if sid == "" or sid == []:
            if (
                ...
                ###楼下不这么折腾清理不干净
                self.net_g, self.cpt = get_synthesizer(cpt, self.config.device)
    ...
    def vc_single(
        ...
        model_error = self._model_not_loaded_error()
