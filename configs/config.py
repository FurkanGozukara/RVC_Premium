...
class Config(metaclass=Singleton):
    ...
    def arg_parse() -> tuple:
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Bind Gradio server host/IP",
        )
        parser.add_argument(
            "--port",
            type=int,
            default=None,
            help="Bind Gradio server port",
        )
        ...
        parser.add_argument(
            "--share",
            action="store_true",
            help="Alias for --global_link (Gradio share link)",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--debug_cuda",
            action="store_true",
            help="Print CUDA environment diagnostics during startup",
        )
        parser.add_argument(
        ...
        if cmd_opts.port is not None and not (0 <= cmd_opts.port <= 65535):
            logger.warning(
                "Ignoring invalid --port value %s; letting Gradio choose the port.",
                cmd_opts.port,
            )
            cmd_opts.port = None

        global_link = cmd_opts.global_link or cmd_opts.share

        return (
            cmd_opts.pycmd,
            cmd_opts.server_name,
            cmd_opts.port,
            global_link,
            cmd_opts.noparallel,
            cmd_opts.noautoopen,
