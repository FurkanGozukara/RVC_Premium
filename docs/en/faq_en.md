## File Structure and Model Placement

### Where to place your voice models (.pth files):
```
RVC_Premium/
├── assets/
│   ├── weights/          ← Put your .pth voice model files here
│   │   └── YourVoice.pth
│   └── indices/          ← Put your .index files here (if you have them)
│       └── YourVoice.index
```

### How to use your voice models:
1. Copy your `.pth` file to `assets/weights/`
2. If you have an index file (`.index`), copy it to `assets/indices/`
3. Launch the RVC application
4. In the "Model Inference" tab, click the **"Refresh voice list and index path"** button
5. Your voice model will appear in the **"Inferencing voice"** dropdown
6. Select your model, upload audio, and click "Convert"

### What is an Index File?
- Index files (.index) are feature databases created during training
- They improve voice quality and timbre accuracy during inference
- The **"Feature searching ratio"** slider controls how much the index influences the output:
  - **0.0** = Don't use index (may sound less accurate)
  - **0.75** = Default balanced mix (recommended)
  - **1.0** = Maximum index usage (strongest timbre matching)
- Models work without index files, but quality may be reduced
